                                    ...
                                    <?php $no = 1; ?>
                                        <?php foreach ($buktipelaporan as $dd) : ?>
                                            ...
                                            <td><?= $no++ ?></td>
                                                ...
                                                <td>
                                                <a href="buktipelaporan" class="btn btn-success btn-sm btn-terima" data-id="<?= $dd['id']; ?>" onclick="return confirm('Apakah Anda yakin ingin menerima data ini?')">Disetujui</a>
                                                <a href="buktipelaporan" class="btn btn-danger btn-sm btn-tolak" data-id="<?= $dd['id']; ?>" onclick="return confirm('Apakah Anda yakin ingin menolak data ini?')">Ditolak</a>


                                                </td>
                                                <td>
                                                    <a href="/buktipelaporan/preview/<?= $dd['id']; ?>" class="btn btn-info btn-sm" >Pratinjau</a>
                                                    <a href="/buktipelaporan/download/<?= $dd['id']; ?>" class="btn btn-success btn-sm">Unduh</a>
                                                    <a href="/buktipelaporan/destroy/<?= $dd['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')">Hapus</a>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        var buttons = document.querySelectorAll('.btn-terima');
        ...
        buttons.forEach(function (button) {
            button.addEventListener('click', function (event) {
                ...
                // Kirim permintaan AJAX ke server untuk mengubah data status_verifikasi
                fetch('/buktipelaporan/terima/' + id, {
                    method: 'GET',
                })
    ...
    document.addEventListener('DOMContentLoaded', function () {
        var buttons = document.querySelectorAll('.btn-tolak');
